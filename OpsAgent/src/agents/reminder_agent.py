...
class ScheduleReminderRequest(BaseModel):
    ...
    delay_seconds: int = Field(
        default=60, description="Delay in seconds before firing reminder"
    )
...
@log_call
def check_and_deliver_pending_reminders(
    slack_say: Optional[Callable] = None, now: Optional[datetime] = None
) -> ReminderCheckResult:
    """
    Polls pending reminders across all users where fire_at <= now and delivered = False, delivers via Slack, and marks delivered.

    Architectural Note:
    Currently operates as three sequential steps (fetch -> deliver -> mark-delivered) designed for single-process runtime.
    If scaling horizontally across multiple bot processes, implement atomic row-claiming or distributed locks
    (e.g., SELECT FOR UPDATE or Redis locks) to avoid duplicate reminder delivery.
    """
    if now is None:
        now = datetime.utcnow()
    ...
    for rem in pending_list:
        ...
        logger.info(
            f"Delivering scheduled reminder #{rem.id} to user {rem.user_id} in channel {rem.channel_id}"
        )
        ...
        if slack_say:
            try:
                ...
                logger.error(
                    f"Failed to send Slack message for reminder #{rem.id}: {e}"
                )
        else:
            logger.info(
                f"[SIMULATED SLACK DELIVERY] Channel: {rem.channel_id} | User: {rem.user_id} | Message: {msg_text}"
            )
...
def start_reminder_scheduler(
    slack_say: Optional[Callable] = None, interval_seconds: int = 15
):
    """
    ...
    if _scheduler is None:
        ...
        _scheduler.start()
        logger.info(
            f"APScheduler started polling for reminders every {interval_seconds}s."
        )
