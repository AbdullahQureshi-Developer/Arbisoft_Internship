...
class ExtractedEvent(BaseModel):
    ...
    start_time: datetime = Field(
        ..., description="Calculated ISO start datetime of the event"
    )
    end_time: datetime = Field(
        ..., description="Calculated ISO end datetime of the event"
    )
    attendees: List[str] = Field(
        default_factory=list, description="Extracted attendee names or emails"
    )
...
def extract_event_details(
    ...
    system_prompt = (
        ...
        f"The current reference local datetime with timezone offset is '{now_iso}'. "
        "Extract the event title, start_time (ISO datetime with local timezone offset), end_time (ISO datetime with local timezone offset), and attendees list. "
        "Calculate start_time and end_time accurately using the reference datetime and preserve the local timezone offset. "
